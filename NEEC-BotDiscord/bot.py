...
CLIENT = os.getenv('CLIENT_SECRET')
#print(TOKEN)
    ...
    for x in users:
        if(str(x.discordUsername[:-2]) == str(member)):
            ...
            if x.access_token != None:
                ...
                r = requests.post(f'https://fenix.tecnico.ulisboa.pt/oauth/refresh_token?client_id=1132965128044744&client_secret=CLIENT==&refresh_token={x.refresh_token}&grant_type=refresh_token')
                #print(r)
                ...
                r = requests.post(f"https://fenix.tecnico.ulisboa.pt/oauth/access_token?client_id=1132965128044744&client_secret=CLIENT==&redirect_uri=http://51.132.30.72:80/&code={code}&grant_type=authorization_code")
                #print(r)
                res = json.loads(r.text)
